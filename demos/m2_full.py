"""M2 demo: full texture — voice-led pad, bass, motif melody, arp, and
Euclidean percussion with phrase-end fills. 64 bars at static mid levers
(PLANS.md M2). The honest DoD test: leave it on for three minutes.

Usage: .venv/bin/python demos/m2_full.py [--seed N] [--bars N] [--tonic C]
       [--mode ionian] [--tension 0.45] [--valence 0.3] [--density 0.55]
       [--roughness 0.35] [--tempo 96] [--no-audio] [--play]
"""
...
ALL_LAYERS = ("pad", "bass", "melody", "arp", "perc")
...
def main() -> None:
    ...
    parser.add_argument("--tempo", type=float, default=96.0)
    ...
    header = (
        f"m2_full │ seed {args.seed} │ {contexts[0].scale.name} │ {args.bars} bars │ "
        f"tension {args.tension} valence {args.valence} density {args.density} "
        f"roughness {args.roughness} │ {config.params.tempo_bpm:g} BPM\n"
    )
    ...
    layer_counts = Counter(e.layer for e in events)
    fills = sum(1 for r in results for line in r.trace if "fill" in line)
    print(header)
    progression = [c.chord_sym for c in contexts]
    for start in range(0, len(progression), config.phrase_bars):
        print("  " + " → ".join(progression[start:start + config.phrase_bars]))
    print(f"\nevents by layer: {dict(layer_counts)} │ fills: {fills}")
    print(f"lint: clean ({len(events)} events) │ round-trip: ok")
    print(f"files: {mid} │ {txt} │ {trace_path}")

    if not args.no_audio:
        wav = audition.render_wav(mid, gain=0.55)
        print(f"audio: {wav}")
        if args.play:
            audition.play(wav)
